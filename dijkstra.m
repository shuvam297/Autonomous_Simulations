...
nodes{2} = [node(1)+1,node(2)];
nodes{3} = [node(1),node(2)-1];
...
% nodes.
    ...
    if (dilatedMap(cell2(1), cell2(2)) ~= -1)
        cost = hypot(cell1(1)-cell2(1),cell1(2)-cell2(2));
    else
...
while idx(1)~=goalIdx(1) || idx(2)~=goalIdx(2) 
