...
p = ones(131) - eye(131);
q = zeros(131);
q(1:3,1:3) = 5*[0.01 0 0;0 0.01 0;0 0 0.01 ];
y  = zeros(3+128*128,1);

  
[laserDataX laserDataY] = bob_getLaserData(connection);
laserDataX = laserDataX.*((0.05*(mod(randn(1,size(laserDataX,2)),2)-1))+1);
laserDataY = laserDataY.*((0.05*(mod(randn(1,size(laserDataX,2)),2)-1))+1);
laser = [laserDataX;laserDataY];
laser = laser(:);
...
while goalDistance > 0.1
    ...
    robotState.x = robotState.x*(1+0.05*(mod(randn(1),2)-1));
    robotState.y = robotState.y*(1+0.05*(mod(randn(1),2)-1));
    robotState.heading = robotState.heading*(1+0.05*(mod(randn(1),2)-1));
    ...
    X= robotState.x;
    Y = robotState.y;
    theta = robotState.heading;
    cs = cos(theta);
    sn = sin(theta);
    tf = [cs, -sn, X; sn, cs, Y; 0, 0, 1];
    
    
    ld = tf*[laserDataX; laserDataY; ones(size(laserDataX))];
    lx=ld(1, :);
    ly=ld(2, :);
    ldd = [lx;ly];
    %lm = reshape(ldd,[size(ldd,1)*size(ldd,2),1]);
    
    z_cap = inv(tf)*[ldd;ones(size(ldd,2),1)];
    z = [laserDataX;laserDataY;ones(size(laserDataX,2),1)];
    IN = z -z_cap;
  
    
    y(1:3,:) = [robotState.x;robotState.y;robotState.heading];
    ...
    ox=min(lx)-mapResolution;
    oy=min(ly)-mapResolution;
    ld=[lx;ly]-repmat([ox;oy], [size(lx) 1]);
    XX=(-2.6)-mapResolution;
    YY=(-2.6)-mapResolution;
    ld1=[lx; ly]-repmat([XX;YY], [size(lx) 1]);
    ld=ld./mapResolution;
    ld1=ld1./mapResolution;
    ld=round(ld);
    ld1=round(ld1);
    ld=ld+ones(size(ld));
    ld1=ld1+2*ones(size(ld));
    Data=zeros(128, 128);
    for i=1:684
        Data(ld(1, i), ld(2, i))=1;
        Data1(ld1(1, i), ld1(2, i))=1;
    end
    
        
    localMap = createMap( [ox, oy], mapResolution, Data );
    imshow(Data1);
    drawnow;
