...
if distToGoal < 1.5*localMap.resolution
    ...
    if isnan(max(max(velTerms)))
        ...
        scores = parameters.headingScoring*headingTermsNormalized + parameters.obstacleDistanceScoring*distTermsNormalized + parameters.velocityScoring*velTermsNormalized;
        
        % Apply smoothing kernel
        scores = filter2(parameters.objectiveFcnSmoothingKernel, scores);
        %scores = imgaussfilt(scores,1.0);
...
% parameters.maxOmegaDot can be used for the maximum deceleration as well.
    ...
    minVel = (robotState.vel-parameters.maxAcc*parameters.timestep);
    maxOmega = min(robotState.omega+parameters.maxOmegaDot*parameters.timestep, parameters.maxOmega);
    minOmega = max(robotState.omega-parameters.maxOmegaDot*parameters.timestep);
...
% Invalid trajectories will get a NaN value assigned.
    ...
    if (stepsBeforeCollision >= nStepsBrake)
        ...
        if ~parameters.globalPlanningOn
            ...
            Y=goalPosition.y-robotStopPose.y;
            X=goalPosition.x-robotStopPose.x;
            goalAngle=atan2(Y, X);
            headingTerm = abs(angleDiff(pi, angleDiff(goalAngle, robotStopPose.heading)));
            %fprintf hahaha;
        else
            ...
            if ~(any(finalTrajectoryIdx > maps.globalGradientMap.size) || any(finalTrajectoryIdx < 1))
                ...
                if isnan(gradientDirection) % that's the case at the goal cell, which is the only extremum in the cost function
                    ...
                    % headingTerm = TODO
                    %headingTerm = abs(angleDiff(pi, angleDiff(gradientDirection.data(robotStopPose.x,robotStopPose.y), robotStopPose.heading)));
                    headingTerm=abs(angleDiff(pi, angleDiff(gradientDirection, robotStopPose.heading)));
                end
            end
        end
    end
